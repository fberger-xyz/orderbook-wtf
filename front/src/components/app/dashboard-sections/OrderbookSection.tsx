...
export default function OrderbookSection() {
    const { currentChainId, selectedTrade, sellToken, getAddressPair } = useAppStore()
        ...
        <div className="flex flex-col w-full border rounded-xl py-3 border-milk-100 gap-1 bg-gray-500/5">
            ...
            <div className="flex flex-col gap-1 text-xs w-full">
                <div className="grid grid-cols-3 px-4 py-0.5 text-milk-400">
                    ...
                    <p className="ml-auto truncate">Total</p>
                </div>
                {asks.length
                    ? asks
                          ...
                          .map((ask, askIndex, slicedAsks) => {
                              ...
                              return (
                                  <div key={`${askIndex}-${ask.amount}`} className="relative group w-full cursor-pointer">
                ...
                {bids.length
                    ? bids.slice(0, ROWS_TO_SHOW).map((bid, bidIndex, slicedBids) => {
                          ...
                          return (
                              <div key={`${bidIndex}-${bid.amount}`} className="relative group w-full cursor-pointer">
